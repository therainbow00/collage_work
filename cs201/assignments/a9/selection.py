...
def my_sort(unsorted):
    ...
    for i in range(len(sorted) - 1):
        index_of_current_smallest = i
        for j in range(i + 1, len(sorted)):
            work += 1
            if sorted[j] < sorted[index_of_current_smallest]:
                 index_of_current_smallest = j
        if index_of_current_smallest != i:
            work += 1
            swap(sorted, index_of_current_smallest, i)
    return sorted, work
