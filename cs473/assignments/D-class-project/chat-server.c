  ...
  for(;;) {
      ...
      while (return_events > 0 && current_poll_fd < MAX_CONNECTIONS)
      {
          if (clientFds[current_poll_fd].revents & POLLIN)
          {
              ...
              // this block checks if the buffer has data to read 
              if (bytesRead <= 0)
              {
                  // if there is no data or there was an error, than the client gets disconnected from the server.
                  fprintf(stdout, "client %d: DISCONNECTED\n"), current_poll_fd;
                  close(clientFds[current_poll_fd].fd);
                  // Once disconnected, the client structure replaces the current pollfd with the last pollfd becasue
                  // we don't want to read data from an empty/disconnected client structure. This is also to prevent any gaps from happening. Than the total number is decreased.
