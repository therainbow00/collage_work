...
void processSum(int argc, char *argv[], struct connection *connList) {
  ...
  /// this part is where you`ll need to set aside some space. In this case, you`re 
  ///   actually managing your own "symbol table" where you need to store both the
  ///   variable name and the value.  This is where having a list of structs containing 
  ///   both names and values comes in handy -- or a pair of in-sync arrays. -aaron
  //variable_name = sum;
...
struct connection *processMessage(char *message, struct connection *connList) {
  ...
  /// you'll want to create a new function to handle the data in this case -aaron
...
int main(int argc, char *argv[]) {
  ...
  // TODO: add passive listening socket to clientFds, increment numberClients
    ...
    /// for conn_get, it looks like you changed your variable name but not its type.
    ///   if we're going to search by fd, we want to use the proper data type (int)
    ///   -aaron

    /// remember that accept returns the file descriptor of a client that is
    ///   CONNECTING to the socket. We actually want to monitor for new connections,
    ///   so we want to make sure our passive socket (returned by the socket command)
    ///   has a special place in our clientFds list. -aaron
  ...
  for(;;) {
      ...
      /// once properly set up, our passive socket will provide POLLIN when a client
      ///   is trying to connect. You'll want to test for POLLIN on your passive socket
      ///   and accept it within the loop. -aaron
      int numberOfFdsWithEvents = poll(clientFds, MAX_CONNECTIONS, -1);
        ...
        /// good for testing connection. remember we'll want to only close our incoming
        ///   connection when a GOODBYE command is issued. -aaron
        do {
